...
import {
  getContador, getProductos, getMarcas,
  postImpresion, patchContador
...
const LabelerForm: React.FC = () => {
  ...
  const handlePrint = async () => {
    ...
    const codigo21 = buildCodigo21({ idLata, lote:form.lote,
      indicador:mkt.indicador, codigoProducto:prod.codigo, pesoGramos:form.peso });
    ...
    /* Airtable */
    await postImpresion({
      id_lata:idLata, lote:+form.lote,
      productoLabel:prod.label, marcaLabel:mkt.label,
      peso:form.peso, rne:prod.rne, rnpa:prod.rnpa,
      fechaFab:form.fechaFab, fechaVto:form.fechaVto, codigo21,
    });
  ...
  /* ---------- render ---------- */
  const summary = `${form.lote} · ${form.fechaFab}/${form.fechaVto} · ${form.peso} g`;
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input type="text" name="lote" value={form.lote} inputMode="numeric" maxLength={5}
            placeholder="#####"
            onChange={e=>{
      ...
      <Modal  open={printerMissing} title="Impresora no encontrada"
        message={'No se detectó BrowserPrint o una impresora Zebra.\n' +
                 'Instalá Zebra Browser Print y recargá la página.'}
        onClose={()=>setPrinterMissing(false)} />

      <ConfirmPrint
        open={confirmOpen}
        summary={summary}
        onKeep ={()=>{ setConfirmOpen(false); setForm(f=>({...f,peso:0})); }}
