...
/* ---------- tipos ---------- */
interface Product {
  id: string; label: string; codigo: string; ingredientes: string;
  marcasIds: string[]; rne: string; rnpa: string;
}
...
const LabelerForm: React.FC = () => {
  ...
  const [lastResumen,    setLastResumen]    = useState('');
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement|HTMLSelectElement>) =>
    setForm(f => ({ ...f, [e.target.name]:
      e.target.name==='peso' ? +e.target.value : e.target.value }));
  ...
  const handlePrint = async () => {
    ...
    /* Airtable */
    await postImpresion({
      id_lata:idLata, lote:+form.lote, marcaLabel:mkt.label,
      productoLabel:prod.label, peso:form.peso,
      rne:prod.rne, rnpa:prod.rnpa, codigo21,
    ...
    /* confirm modal */
    setLastResumen(`${prod.label} / ${mkt.label}\nF. Fab: ${form.fechaFab}\nLote: ${form.lote}`);
    setShowConfirm(true);
  };
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <Dropdown
          ...
          value={form.marcaId}
          onChange={v=>setForm(f=>({...f,marcaId:v,productoId:''}))}
        />
        {/* select producto */}
        ...
        <Dropdown
          ...
          onChange={v=>setForm(f=>({...f,productoId:v}))}
        ...
        <div>
          ...
          <input
            name="lote" maxLength={5} placeholder="#####"
            value={form.lote} inputMode="numeric" pattern="\d{5}"
        ...
        <div className="grid grid-cols-2 gap-4">
          <input type="date" name="fechaFab" value={form.fechaFab}
            onChange={handleChange}
            className="mt-1 block w-full rounded-xl border border-neutral-200 px-3 py-2" />
          <input type="date" name="fechaVto" value={form.fechaVto}
            onChange={handleChange}
            className="mt-1 block w-full rounded-xl border border-neutral-200 px-3 py-2" />
        </div>
      ...
      <Modal
        ...
        message={`No se detectó BrowserPrint o una impresora Zebra.\n`+
                `Instalá Zebra Browser Print y volvé a intentar.`}
