...
import { Info } from 'lucide-react';     // ícono chiquito

interface Props {
  online : boolean;          // true = conectada
  onRetry: () => void;       // click => reintentar detección
}

const PrinterStatus: React.FC<Props> = ({ online, onRetry }) => (
  <div
    onClick={onRetry}
    title={online ? 'Impresora conectada' :
                    'Impresora no detectada. Click para reintentar'}
