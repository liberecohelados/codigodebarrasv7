...
/* ---------- PRODUCTOS (nombre, ingredientes, link a marcas) ---------- */
export async function getProductos() {
  const recs = await base('productos')
    .select({
      fields: [
        'nombre producto',   // exactamente como figura en Airtable
        'codigo_producto',
        'rne',
        'rnpa',
        'ingredientes',
        'Link a Marcas',     // campo “Link to another record”
      ],
    })
    .all();
  ...
  return recs.map(r => ({
    ...
    marcasIds: (r.fields['Link a Marcas'] || []) as string[], // ← IDs de marcas
